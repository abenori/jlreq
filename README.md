...
### `\warichu`
割注を出力します．行分割位置などは自動で計算されます．（複数回のコンパイルが必要．）`\inlinenote*`ではこれらの位置を手動で指定できます．書式は
...
* `paper=[a3paper/a4paper/a5paper/a6paper/b4paper/b5paper/b6paper/a4j/a5j/b4j/b5j/a4var/b5var/letterpaper/legalpaper/executivepaper/{<寸法>,<寸法>}]`：紙サイズです．紙サイズ名を指定する他，`{<縦>,<横>}`と直接寸法を指定することもできます．
...
* `gutter=<寸法;zh,zw>` : のどの余白の大きさ．奇数ページにおける右余白，偶数ページにおける左余白．デフォルトは中央寄せになるような値．
