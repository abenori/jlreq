...
### `trimmarks_pattern`
トンボの形．直接の描画コードを指定します．次のように`centers`に指定すると，上下左右の真ん中のトンボの形が変わります．指定するのは上の真ん中のトンボの描画コードであり，そのほかの場所にはそれを回転したものが描画されます．なお，指定前に`graphicx`パッケージを読み込んでおく必要があります．
```latex
\jlreqtrimmarkssetup{
  trimmarks_pattern = {
    centers = {
